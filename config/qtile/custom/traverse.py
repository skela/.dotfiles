...
Qtile versions known to work: 0.29.1
"""
...
def focusable(qtile):
	...
	yield from (window for screen in qtile.screens for window in screen.group.windows if window is not qtile.current_window)
	yield from (
		screen for screen in qtile.screens if not screen.group.windows    # only consider empty screens
...
class Rectangle:
	...
	def center(self) -> Point:
		...
		return Point(
			self.main + int(self.length / 2),
			self.cross + int(self.breadth / 2),
		)
...
def _focus_window(qtile, transform) -> Optional[Window | Screen]:
	...
	candidates = filter(lambda candidate: candidate.center.main > focused.center.main + 5 and candidate.is_visible, map(transform, focusable(qtile)))
	...
	closest = sorted(
		...
		key=lambda candidate: (
			0 if band_min <= candidate.cross <= band_max or band_min <= candidate.cross + candidate.breadth <= band_max else 1,
